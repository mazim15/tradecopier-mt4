...
// Trading Days Parameters
extern bool   UseTradingDays = true;   // Enable/Disable specific trading days
extern bool   MondayTrading = true;    // Allow trading on Monday
extern bool   TuesdayTrading = true;   // Allow trading on Tuesday
extern bool   WednesdayTrading = true; // Allow trading on Wednesday
extern bool   ThursdayTrading = true;  // Allow trading on Thursday
extern bool   FridayTrading = true;    // Allow trading on Friday
...
void OnTick()
{
    ...
    if(UseTradingDays && !IsTradingAllowedToday())
    {
        statusMsg = "Trading paused - Trading not allowed on " + DayOfWeekAsString(DayOfWeek());
    }
    else if(UseTimer && !IsWithinTradingHours())
    {
    ...
    comment += "--- Trading Days ---\n";
    if(UseTradingDays)
    {
        comment += "Monday: " + (MondayTrading ? "Enabled" : "Disabled") + "\n";
        comment += "Tuesday: " + (TuesdayTrading ? "Enabled" : "Disabled") + "\n";
        comment += "Wednesday: " + (WednesdayTrading ? "Enabled" : "Disabled") + "\n";
        comment += "Thursday: " + (ThursdayTrading ? "Enabled" : "Disabled") + "\n";
        comment += "Friday: " + (FridayTrading ? "Enabled" : "Disabled") + "\n";
        comment += "Current Day: " + DayOfWeekAsString(DayOfWeek()) + "\n\n";
    }
    else
    {
        comment += "Trading Days Filter Disabled - Trading all days\n\n";
    }
    ...
    if(UseTradingDays && !IsTradingAllowedToday()) return;
...
{
    if(!UseTradingDays) return true;
    
    int currentDayOfWeek = DayOfWeek();
    
    switch(currentDayOfWeek)
        {
            case 1: return MondayTrading;
            case 2: return TuesdayTrading;
            case 3: return WednesdayTrading;
            case 4: return ThursdayTrading;
            case 5: return FridayTrading;
        case 6: return true; // Sunday
        default: return false; // Weekend (Saturday=0, Sunday=6)
        }
}
...
//+------------------------------------------------------------------+
string DayOfWeekAsString(int dayOfWeek)
{
    switch(dayOfWeek)
    {
